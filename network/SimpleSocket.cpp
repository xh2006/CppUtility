...
void CSimpleSocket::CreateMsgServerTcp(std::string strIP, unsigned short uPort)
{
    ...
    while (!m_bStop)
    {
        ...
        if (m_workPattern == np_server)
        {
            ...
            std::thread th(&CSimpleSocket::RecvData, this, std::ref(connSock));
            th.detach();
        }
        else if (m_workPattern == np_server_ex)
        {
            AddSocketInfo(connSock);
            std::thread th(&CSimpleSocket::RecvDataEx, this, std::ref(connSock));
            th.detach();
        }
#else
    ...
    while (true)
    {
        ...
        if (nRecvSize > 0)
        {
            ...
            while (true)
            {
                ...
                {
                    Log("[network] RecvData, number: %lld, size : %d \n", ++m_nRecvCount, nRecvDataSize);
...
bool CSimpleSocket::SendData(const socket_r& s, int dataType, const char* pData, int nDataSize)
{
    ...
    if (s != INVALID_SOCKET)
    {
        ...
        Log("[network] SendData, number: %lld, size: %d \n", ++m_nSendCount, smh.headerSize + smh.dataSize);
        bRet = true;
        if (iResult == SOCKET_ERROR)
        {
            bRet = false;
